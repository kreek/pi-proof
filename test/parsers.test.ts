...
import { parseTestOutput, formatDuration, defaultParsers, type TestLineParser } from "../src/parsers.js";
...
describe("individual parsers", () => {
	...
	describe("pytest", () => {
		it("parses passing test", () => {
			expect(parseLine("pytest", "test_math.py::test_add PASSED")).toEqual({ name: "test_math.py::test_add", passed: true });
		});

		it("parses failing test", () => {
			expect(parseLine("pytest", "test_math.py::test_add FAILED")).toEqual({ name: "test_math.py::test_add", passed: false });
		});
	...
	describe("cargo", () => {
		it("parses passing test", () => {
			expect(parseLine("cargo", "test math::tests::test_add ... ok")).toEqual({ name: "math::tests::test_add", passed: true });
		});

		it("parses failing test", () => {
			expect(parseLine("cargo", "test math::tests::test_add ... FAILED")).toEqual({ name: "math::tests::test_add", passed: false });
		});
	});
	...
	describe("jest-vitest with bracket duration (Bun)", () => {
		it("parses passing test with bracket duration", () => {
			expect(parseLine("jest-vitest", "✓ adds two numbers [0.06ms]")).toEqual({ name: "adds two numbers", passed: true });
		});
	...
	describe("python-unittest", () => {
		it("parses passing test", () => {
			expect(parseLine("python-unittest", "test_add (test_math.TestMath) ... ok")).toEqual({ name: "test_math.TestMath.test_add", passed: true });
		});

		it("parses failing test", () => {
			expect(parseLine("python-unittest", "test_divide (test_math.TestMath) ... FAIL")).toEqual({ name: "test_math.TestMath.test_divide", passed: false });
		});

		it("parses error test", () => {
			expect(parseLine("python-unittest", "test_divide (test_math.TestMath) ... ERROR")).toEqual({ name: "test_math.TestMath.test_divide", passed: false });
		});
	});

	describe("minitest", () => {
		it("parses passing test", () => {
			expect(parseLine("minitest", "TestMath#test_add = 0.00 s = .")).toEqual({ name: "TestMath#test_add", passed: true });
		});

		it("parses failing test", () => {
			expect(parseLine("minitest", "TestMath#test_add = 0.01 s = F")).toEqual({ name: "TestMath#test_add", passed: false });
		});

		it("parses error test", () => {
			expect(parseLine("minitest", "TestMath#test_add = 0.00 s = E")).toEqual({ name: "TestMath#test_add", passed: false });
		});
	});
	...
	describe("gradle", () => {
		...
		it("parses failing test", () => {
			expect(parseLine("gradle", "MathTest > testDivide() FAILED")).toEqual({ name: "MathTest.testDivide()", passed: false });
		});
	});

	describe("xctest", () => {
		it("parses passing test", () => {
			expect(parseLine("xctest", "Test Case '-[MathTests testAdd]' passed (0.001 seconds).")).toEqual({ name: "MathTests.testAdd", passed: true });
		});

		it("parses failing test", () => {
			expect(parseLine("xctest", "Test Case '-[MathTests testDivide]' failed (0.002 seconds).")).toEqual({ name: "MathTests.testDivide", passed: false });
		});
	});
	...
	describe("exunit", () => {
		it("parses passing test", () => {
			expect(parseLine("exunit", "  * test adds two numbers (0.1ms)")).toEqual({ name: "adds two numbers", passed: true });
		});
